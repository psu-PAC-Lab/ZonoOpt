...
def test_operator_overloading():
    ...
    assert type(p).__name__ == 'Point' and id(p) == p_id, "Point += Point: expected in-place Point"

    # Point += Zono  → Zono (type promotion)
    p = zono.Point(c3)
    expected = zono.minkowski_sum(p, Z1)
    p += Z1
    assert type(p).__name__ == 'Zono', f"Point += Zono: expected Zono, got {type(p).__name__}"
    ...
    p += cz
    assert type(p).__name__ == 'ConZono', f"Point += ConZono: expected ConZono, got {type(p).__name__}"
    ...
    hz = make_hybzono([0., 0.])
    p += hz
    assert type(p).__name__ == 'HybZono', f"Point += HybZono: expected HybZono, got {type(p).__name__}"

    # Point += Box  → Zono
    p = zono.Point(c3)
    expected = zono.minkowski_sum(p, zono.interval_2_zono(box))
    p += box
    assert type(p).__name__ == 'Zono', f"Point += Box: expected Zono, got {type(p).__name__}"
    ...
    z_id = id(z_op)
    z_op += Z2
    assert type(z_op).__name__ == 'Zono' and id(z_op) == z_id, "Zono += Zono: expected in-place Zono"

    # Zono += Point  → Zono, in-place
    z_op = Z1.copy()
    z_id = id(z_op)
    z_op += P3
    assert type(z_op).__name__ == 'Zono' and id(z_op) == z_id, "Zono += Point: expected in-place Zono"
    ...
    z_op += cz
    assert type(z_op).__name__ == 'ConZono', f"Zono += ConZono: expected ConZono, got {type(z_op).__name__}"
    ...
    hz = make_hybzono([0., 0.])
    z_op += hz
    assert type(z_op).__name__ == 'HybZono', f"Zono += HybZono: expected HybZono, got {type(z_op).__name__}"

    # ConZono += ConZono  → ConZono, in-place
    cz1 = make_conzono([1., 2.])
    cz1_id = id(cz1)
    cz1 += make_conzono([0., 0.])
    assert type(cz1).__name__ == 'ConZono' and id(cz1) == cz1_id, "ConZono += ConZono: expected in-place ConZono"

    # ConZono += Zono  → ConZono, in-place
    cz1 = make_conzono([1., 2.])
    cz1_id = id(cz1)
    cz1 += Z1
    assert type(cz1).__name__ == 'ConZono' and id(cz1) == cz1_id, "ConZono += Zono: expected in-place ConZono"

    # ConZono += HybZono  → HybZono
    cz1 = make_conzono([1., 2.])
    hz = make_hybzono([0., 0.])
    cz1 += hz
    assert type(cz1).__name__ == 'HybZono', f"ConZono += HybZono: expected HybZono, got {type(cz1).__name__}"

    # Point *= scalar  → Point, in-place
    p = zono.Point(c3)
    p_id = id(p)
    p *= 2.0
    assert type(p).__name__ == 'Point' and id(p) == p_id, "Point *= scalar: expected in-place Point"
    ...
    p_id = id(p)
    p -= np.array([1., 2.])
    assert type(p).__name__ == 'Point' and id(p) == p_id, "Point -= vector: expected in-place Point"
