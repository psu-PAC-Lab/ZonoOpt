...
{
    return 2.*std::pow(std::tan(x[0]), 3) + std::cos(x[1])/3. + std::sin(x[0] + std::atan(x[2])) + std::exp(x[4]/x[3]) - std::acos(x[5])*std::asin(x[6]);
}
...
Interval f_int(const Box& x)
{
    ...
    return (x[0].tan().pow(3))*2. + x[1].cos()/3. + (x[0] + x[2].arctan()).sin() + (x[4]/x[3]).exp() - (x[5].arccos() * x[6].arcsin());
}
